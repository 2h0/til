...
# 15回目 フッターのリンクを完成させよう
余白を付ける方法ですが margin も padding もそうなのですが、
実は 3 番目の数値を付けると、それが下方向の値になる

透明度は opacity というプロパティを操作
60% の透明度にしたいなら
opacity: 0.6;

# 16回目 floatで要素の位置を変えてみよう
テキストとテキストの高さを揃えたい場合
float というプロパティを使う

float というのはその名前の通り、
通常の要素より浮かせて配置するという意味で
float: left;
としてあげると左側に浮いた感じになる
